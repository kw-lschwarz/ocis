...
`FRONTEND_USER_SEARCH_DISPLAYED_ATTRIBUTES` +
...
++[] ++
a| [subs=-attributes]
A list of user attributes to display in the user search results.
...
a|`OCIS_CACHE_STORE` +
xref:deprecation-note-2026-02-20-08-47-03[Deprecation Note]
a| [subs=-attributes]
++pre5.0 ++
a| [subs=-attributes]
++string ++
...
a|`FRONTEND_CONFIGURABLE_NOTIFICATIONS` +
...
a|`OCIS_CLAIM_MANAGED_SPACES_ENABLED` +
...
++false ++
a| [subs=-attributes]
Set to true to enable multi factor authentication. See the documentation for more details.

a|`OCIS_MFA_AUTH_LEVEL_NAMES` +
