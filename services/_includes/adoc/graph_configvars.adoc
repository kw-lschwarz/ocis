...
[#deprecation-note-2026-02-20-08-47-03]
[caption=]
...
a|`OCIS_USER_SEARCH_DISPLAYED_ATTRIBUTES` +
...
++[] ++
a| [subs=-attributes]
The attributes to display in the user search results.
...
a|`OCIS_LDAP_USER_SCHEMA_EXTERNAL_ID` +
...
a|`GRAPH_LDAP_REQUIRE_EXTERNAL_ID` +
...
a|`OCIS_LDAP_USER_MEMBER_ATTRIBUTE` +
...
a|`OCIS_LDAP_USER_GUEST_ATTRIBUTE` +
...
a|`OCIS_LDAP_GROUP_AFFILIATION_ATTRIBUTE` +
...
++ ++
a| [subs=-attributes]
LDAP Attribute to signal which instance the group is belonging to. Requires OCIS_MULTI_INSTANCE_ENABLED.

a|`OCIS_LDAP_PRECISE_SEARCH_ATTRIBUTE` +
...
a|`OCIS_LDAP_INSTANCE_MAPPER_ENABLED` +
...
a|`OCIS_LDAP_INSTANCE_MAPPER_BASE_DN` +
...
a|`OCIS_LDAP_INSTANCE_MAPPER_NAME_ATTRIBUTE` +
...
a|`OCIS_LDAP_INSTANCE_MAPPER_ID_ATTRIBUTE` +
...
a|`OCIS_LDAP_CROSS_INSTANCE_REFERENCE_TEMPLATE` +
...
a|`OCIS_LDAP_INSTANCE_URL_TEMPLATE` +
...
a|`OCIS_MULTI_INSTANCE_INSTANCEID` +
...
a|`OCIS_MULTI_INSTANCE_QUERY_TEMPLATE` +
